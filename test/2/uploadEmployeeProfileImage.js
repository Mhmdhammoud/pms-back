...
describe('Uploading an employee profile image ', () => {
	before((done) => {
		connectDB()
			.then(() => done())
			.catch((err) => done(err));
	});
	it('FAIL, Upload employee profile image without employee Login', (done) => {
		const buffer = Buffer.from('../storm.jpg');
		request(app)
			.put('/api/v1/employee/profile/image')
	...
	it('FAIL, Upload employee profile image with manager Login', (done) => {
		const buffer = Buffer.from('../storm.jpg');
		request(app)
	...
	it('PASS,  Upload employee profile image with employee Login', (done) => {
		const buffer = Buffer.from('../storm.jpg');
		request(app)
			...
			.then(({body}) => {
				...
				request(app)
					...
					.then(({body: uploadBody}) => {
						...
						emptyS3Directory(`PMS/employee/${body.id}`).then(() => {
							done();
						});
					})
					.catch((err) => done(err));
